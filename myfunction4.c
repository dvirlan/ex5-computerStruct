 ...
 */
static pixel applyKernel(int dim, int i, int j, pixel *src, int kernelSize, int kernelScale, bool filter) {
...
void smooth(int dim, pixel *src, pixel *dst, int kernelSize, int kernelScale, bool filter) {
    ...
    for (i = 1 ; i < local_n - 1; i++) { //kernelSize / 2 = 1
        for (j =  1; j < local_n - 1 ; j++) {
            dst[calcIndex(i, j, local_n)] = applyKernel(local_n, i, j, src, kernelSize, kernelScale, filter);
        }
    }
}
